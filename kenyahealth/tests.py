...
from .models import Doctor,History,Patient
...
class DoctorTestClass(TestCase):
    ...
    def test_instance(self):
        self.assertTrue(isinstance(self.manulangat,Doctor))
...
class HistoryTestClass(TestCase):
    def setUp(self):
        ...
        self.hist.save()
    def test_instance(self):
        self.assertTrue(isinstance(self.hist,History))
...
class PatientTestClass(TestCase):
    def setUp(self):
        ...
        self.pat.save()
    def test_instance(self):
        self.assertTrue(isinstance(self.pat,Patient))
    def test_save_pati(self):
        self.pat.save_pati()
        pati = Patient.objects.all()
        self.assertTrue(len(pati) > 0)
    def tearDown(self):
        User.objects.all().delete()
        Doctor.objects.all().delete()
        History.objects.all().delete()
